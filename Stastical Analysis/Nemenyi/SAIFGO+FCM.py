...
# ==========================================================
# 3️⃣ MAIN EXECUTION (NEMENYI STATS & PLOTS)
# ==========================================================
...
if __name__ == "__main__":
    ...
    if not os.path.exists(file_path):
        ...
        for i in range(len(features)):
            ...
            # Check if clusters actually have items before testing
        ...
        if len(significant_results) == 0:
            ...
            for idx, (feature_name, nemenyi_matrix) in enumerate(significant_results):
                ...
                plt.imshow(nemenyi_matrix, cmap='viridis')
                plt.colorbar()
                plt.title(f"Nemenyi Post-Hoc - {feature_name} (SAIFGO+FCM)")
                plt.xticks(range(len(clusters)), [f"C{c}" for c in clusters])
                plt.yticks(range(len(clusters)), [f"C{c}" for c in clusters])

            plt.tight_layout()
            save_path = os.path.join(os.getcwd(), "SAIFGO+FCM_Nemenyi.jpg")
